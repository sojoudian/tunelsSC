...
func DialContext(ctx context.Context, addr, typ string, tlsCfg *tls.Config) (conn *loggedConn, err error) {
	dialer := &net.Dialer{}
...
// Dial maintains backward compatibility by using context.Background()
func Dial(addr, typ string, tlsCfg *tls.Config) (conn *loggedConn, err error) {
	return DialContext(context.Background(), addr, typ, tlsCfg)
}

func DialHttpProxyContext(ctx context.Context, proxyUrl, addr, typ string, tlsCfg *tls.Config) (conn *loggedConn, err error) {
	...
	// dial the proxy
	if conn, err = DialContext(ctx, parsedUrl.Host, typ, proxyTlsConfig); err != nil {
	...
	if err != nil {
		err = fmt.Errorf("failed to create CONNECT request: %w", err)
		return
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to read proxy response: %w", err)
		return
	}
	resp.Body.Close()
...
// DialHttpProxy maintains backward compatibility by using context.Background()
func DialHttpProxy(proxyUrl, addr, typ string, tlsCfg *tls.Config) (conn *loggedConn, err error) {
	return DialHttpProxyContext(context.Background(), proxyUrl, addr, typ, tlsCfg)
}
...
func JoinContext(ctx context.Context, c Conn, c2 Conn) (int64, int64) {
...
// Join maintains backward compatibility by using context.Background()
func Join(c Conn, c2 Conn) (int64, int64) {
	return JoinContext(context.Background(), c, c2)
}
